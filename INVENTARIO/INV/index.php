...
<?php
	...
	echo $diseno;
	$total_3 = 0;
	$tabla = "<table id='irvin' ><tr><th>ID</th><th>Productos</th><th>Sistema</th><th>Fisico</th><th>Dif</th></tr>";
	...
	while($row = mysqli_fetch_array($bdq))
	{
		...
		$unidad = $row['unidad'];
		$ventas_diarias = "SELECT unidad,nombre,id_ingrediente,COUNT(cantidad)as total2,SUM(cantidad)as masas from ventas_invt t2  left join productos using(id_producto) where date(fecha_creacion) between '$fecha_inicio' and '$fecha' AND t2.id_ingrediente=$id and unidad=$unidad group by t2.id_ingrediente order by nombre";
		...
		{	
			if($id == 63)
			{
				$tabla .= "<tr><td>".$row['id_ingrediente']."</td><td>".$row['nombre']." ".$row['nombre_unidad']."</td><td>".$total_1 =round(($row['total'] - $row2['masas']),2)."</td>";
			...
			elseif($unidad = $row2['unidad']){
			$tabla .= "<tr><td>".$row['id_ingrediente']."</td><td>".$row['nombre']." ".$row['nombre_unidad']."</td><td>".$total_1 =round(($row['total'] - $row2['total2']),2)."</td>";
			while($row4 = mysqli_fetch_array($bdq4)){
			$tabla .= "<td>".$total_2 = ($row4['total_sucursal'])."</td>";
			$tabla .= "<td>".round($total_1 - $total_2,2)."</td></tr>";
			}
	...
	while($row3 = mysqli_fetch_array($bdq3)){
		...
		if ($row3['unidad']== $unidad){
			$tabla .= "<tr bgcolor='#FF0000'><td>".$row3['id_ingrediente']."</td><td title='no se han movido desde ultimo abastecimiento' bgcolor='#FF0000' >".$row3['nombre']." ".$row3['nombre_unidad']."</td><td>".$total_4 = ($row3['total'])."</td>";
		}
