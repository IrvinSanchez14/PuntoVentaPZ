...
function estadisticas() {
    ...
    rsv_solicitar('estadisticas',{periodo_inicio: periodo_inicio, periodo_final: periodo_final},function(datos){
		...
		if (typeof datos.aux.venta_por_prom != 'undefined') {
           var buffer = '';
   
           for (prom in datos.aux.venta_por_prom)
           {
              buffer += "<li>" +  + "</li>";
           }
           agregar_info('<h1>Total PROMEDIO</h1>');
           agregar_info('<ul>' + buffer + '</ul>');
        } else {
           agregar_info('Sin datos de rendimiento PROMEDIO');
        }
		...
		if (typeof datos.aux.cuentas_por_dias != 'undefined') {
           var buffer = '';
   
           for (dia in datos.aux.cuentas_por_dias )
           {
              buffer += "<li>" + datos.aux.cuentas_por_dias[dia].horas + " - " + datos.aux.cuentas_por_dias[dia].total + "</li>";
           }
           agregar_info('<h1>Cuentas abiertas por dias</h1>');
           agregar_info('<ul>' + buffer + '</ul>');
        } else {
           agregar_info('Sin datos de rendimiento de cuentas por dias');
        }
