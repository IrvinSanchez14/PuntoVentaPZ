...
$c = 'SELECT  DATE(fechahora_pedido) AS dia, '.$c_total.', COUNT(ID_cuenta) AS cue FROM `pedidos` AS t2  LEFT JOIN `cuentas` USING(ID_cuenta) 
WHERE `fechahora_pagado` 
BETWEEN "'.$periodo_inicio.'" AND "'.$periodo_final.'" AND flag_pagado=1 AND flag_anulado=0 AND flag_cancelado=0 
GROUP BY DATE(fechahora_pagado) ORDER BY DATE(fechahora_pagado) DESC';
$r = db_consultar($c);
while ($f = db_fetch($r))
{
    $json['aux']['venta_por_prom'][] = $f;
}
...
/************************************************/
// Cuentas (mesas servidas) por DIA
$c= 'SELECT  DATE_FORMAT(`fechahora_pagado`, "%Y-%M-%d") AS horas, COUNT(ID_cuenta)AS total
FROM cuentas WHERE DATE(`FECHAHORA_PAGADO`)
BETWEEN "'.$periodo_inicio.'" AND "'.$periodo_final.'" AND flag_anulado =0
GROUP BY DATE(`fechahora_pagado`)';
$r = db_consultar($c);

while ($f = db_fetch($r))
{
    $json['aux']['cuentas_por_dias'][] = $f;
}
